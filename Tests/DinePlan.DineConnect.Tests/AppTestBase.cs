...
using DinePlan.DineConnect.Tests.TestDatas;
...
namespace DinePlan.DineConnect.Tests
{
    ...
    /// Provides methods to easily work with <see cref="DineConnectDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<DineConnectTestModule>
    {
        ...
        protected void UsingDbContext(Action<DineConnectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<DineConnectDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<DineConnectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<DineConnectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DineConnectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DineConnectDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<DineConnectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DineConnectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<DineConnectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DineConnectDbContext, Task<T>> func)
        {
            T result;
